...
static NSString *const cellIdentifier = @"segmentCell";

@interface WYSegmentControlView ()<UICollectionViewDelegateFlowLayout,UICollectionViewDataSource>
...
@implementation WYSegmentControlView
...
{
    WYSegmentCell *cell = (WYSegmentCell*)[self.collectionView cellForItemAtIndexPath:indexPath];
    cell.selected = YES;
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didClickedOneIndex:)]) {
        [self.delegate didClickedOneIndex:self.currentIndex];
    }
...
{
    WYSegmentCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
...
{
    WYSegmentCell *cell = (WYSegmentCell*)[collectionView cellForItemAtIndexPath:indexPath];
    cell.selected = NO;
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_titles.count>5) {
        size = CGSizeMake(size.width+26, CGRectGetHeight(self.bounds));
    }
    else
    {
        size = CGSizeMake([UIScreen mainScreen].bounds.size.width / _titles.count, CGRectGetHeight(self.bounds));
    }
    return size;
}
...
- (UIScrollView*)rootScrollView
{
    if (!_rootScrollView) {
        ...
        [self.parentViewController.view addSubview:_rootScrollView];//self.parentViewController must has a value
    }
...
- (UICollectionView*)collectionView
{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[WYSegmentCell class] forCellWithReuseIdentifier:cellIdentifier];
